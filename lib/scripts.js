...
const run = async function(
  ...
  // clean up
  log.info('scripts.run.cleanup');
  const scriptOptions = files.readOptions(files.getExisting(
    files.SCRIPT_OPTIONS_FILE_NAME, [scriptDirectory, inputDirectory]));
  log.info('keepBrowser:' + scriptOptions.keepBrowser);
  log.info({ options: scriptOptions }, "scripts.options");
...
const instantiateBrowserContext = async function(contextName,
  ...
  log.info('scripts.instantiateBrowserContext outputContextDirectory: ' + outputContextDirectory);
  // options
...
const prepare4Warc = async function(
    browserContextOptions, runOptions, outputContextDirectory) {
  log.info('scripts.prepare4Warc begin');
  log.info("scripts.prepare4Warc runOptions[runopts.WARC]: " + runOptions[runopts.WARC] + 'runOptions[runopts.REPLAY]: ' + runOptions[runopts.REPLAY] + 'runopts.REPLAY_NOT: ' + runopts.REPLAY_NOT);
  if (runOptions[runopts.WARC]
    ...
    log.info('scripts.prepare4warc.pywbInitialize');
    await pywb.initialize(pywbDirectory);
    log.info("scripts.prepare4Warc.preparePywb");
...
const prepare4Replay = async function(
    browserContextOptions, runOptions, outputContextDirectory,
    scriptDirectory, inputDirectory) {
  log.info('scripts.prepare4Replay begin');
  if (runOptions[runopts.REPLAY] !== runopts.REPLAY_NOT) {
    ...
    log.info('scripts.prepare4Replay sourcePywbDirectories.length:' + sourcePywbDirectories.length);
