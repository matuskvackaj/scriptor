...
const run = async function(
  ...
  log.info('mk.scripts.run.options');
  ...
  log.info('mk.keepBrowser:' + scriptOptions.keepBrowser);
  log.info({ options: scriptOptions }, "mk.script.options");
  
  if(!scriptOptions.keepBrowser){
    ...
    return Promise.all(Object.keys(browserContexts).map(async function(contextName) {
      ...
      return browserContext.close().then(_ => {
        ...
        log.info("mk.scripts.run.contextClosed");
      });
    })).then(_ => {
...
const source = function(scriptDirectory) {
  ...
  log.info("mk.scripts.source.execute");
...
const instantiateBrowserContexts = async function(
  ...
  log.info('mk.scripts.instantiateBrowserContexts.names');
...
const instantiateBrowserContext = async function(contextName,
  ...
  log.info("scripts.instantiateBrowserContext.options.read");
  fs.writeJsonSync(
  ...
  log.info('scripts.instantiateBrowserContext.options.final');
  // launch
  ...
  if (warcProcess !== null) {
    browserContext.on("close", () => {
      ...
      log.info("mk.scripts.onClose.terminateWarcProcess");
      warcProcess.kill();
    });
  }
  ...
  log.info('mk.scripts.instantiateBrowserContext.done');
...
const prepare4Har = function(
  ...
  if (runOptions[runopts.HAR]) {
    ...
    log.info("mk.scripts.prepare4Har");
...
const prepare4Tracing = function(
  ...
  if (runOptions[runopts.TRACING]) {
    ...
    log.info("mk.scripts.prepare4Tracing");
...
const prepare4Replay = async function(
  ...
  if (runOptions[runopts.REPLAY] !== runopts.REPLAY_NOT) {
    ...
    if (sourcePywbDirectories.length > 0) {
      ...
      await pywb.initialize(outputPywbDirectory);
    }
 ...
 * @param {string} warcInput - WARC file or directory of these to add to the
 * pywb collection (default: undefined)
 */
const preparePywb = async function(
    browserContextOptions, record, pywbDirectory, warcInput = undefined) {
  log.info("scripts.preparePywb");
  log.info("mk.scripts.preparePywb");

  if (warcInput !== undefined) {
    const collectionDirectory =
      path.join(pywbDirectory, pywb.COLLECTION_DIRECTORY)
    log.info({
      warcInput: warcInput,
      collectionDirectory: collectionDirectory
    }, "scripts.preparePywb.copyWarcInput");
    if (fs.lstatSync(warcInput).isDirectory()) {
      fs.readdirSync(warcInput).forEach(inputFile => {
        fs.copySync(
          path.join(warcInput, inputFile),
          path.join(collectionDirectory, path.basename(inputFile)));
      });
    } else {
      fs.copySync(
        warcInput,
        path.join(collectionDirectory, path.basename(warcInput)));
    }
  }
  ...
  log.info('mk.scripts.preparePywb.started');
...
const startTracing = async function(
  ...
  if (runOptions[runopts.TRACING]) {
    ...
    log.info('mk.scripts.startTracing')
...
const stopTracing = async function(
  ...
  if (runOptions[runopts.TRACING]) {
    ...
    log.info("mk.scripts.stopTracing");
...
const startUnrandomize = async function(browserContext, runOptions) {
  ...
  if (value === runopts.UNRANDOMIZE_NOT) {
    ...
    log.info('mk.scripts.startUnrandomize.constant');
    // https://xkcd.com/221/
