...
const getRunOptions = function(options) {
  log.info('MK options on input: ' + options);
  const runOptions = {};
  ...
  if(options.closeBrowser !== undefined){
    runopts.setCloseBrowser(runOptions);
  }
  if(options.images !== undefined){
    runopts.setImages(runOptions);
  }
  if(options.snapshot !== undefined){
    runopts.setSnapshot(runOptions);
  }
  log.info({
    options: options,
